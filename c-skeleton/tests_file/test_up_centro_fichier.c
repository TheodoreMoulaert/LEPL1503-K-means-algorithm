...
void test_up_centro_f1() {
    ...
    for (int i = 0; i < K; i++){
        clusters[i].size = vector_count;//size_clusters(clusters[i]); // Taille de chaque cluster
    ...
    clusters[1].data[3].coords[1] = (int64_t)7;

    clusters[1].data[4].coords[0] = (int64_t)3;
    clusters[1].data[4].coords[1] = (int64_t)5;

    clusters[1].data[5].coords[0] = (int64_t)5;
