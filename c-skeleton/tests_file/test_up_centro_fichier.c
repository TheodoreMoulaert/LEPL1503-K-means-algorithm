...
void test_up_centro_f1() {
    ...
    uint64_t size_clu = size_clusters(vectors); // Nombre de clusters
    uint32_t dim = vectors[0]->dim; // Dimension des points
    uint64_t vector_count = vectors[0]->nbr_vector; // Nombre de vecteurs

    // Déclaration des clusters
    cluster_t clusters[vector_count];
