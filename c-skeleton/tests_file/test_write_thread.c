    ...
    for (int i = 0; i < nombre_comb; i++) {
        ...
        for (int j = 0; j < k; j++) {
            ...
            for (int d = 0; d < dimension; d++) {
                centroid_init_Array[j].coords[d] = i + j + d; // Valeurs arbitraires pour le test
            ...
            for (int l = 0; l <= j; l++) {
                clustersArray[i][j].data[l] = &centroid_init_Array[j]; // Utilisation des mêmes points pour le test
            }
        }
    }
