...
#include "../headers/update_centroids.h" // Ajout de l'en-tête pour la fonction update_centroids
...
void test_update_centroids() {
    ...
    update_centroids(clusters_input, k);

    // Affichage des clusters et des centroides mis à jour
    printf("\nClusters avec les centroides mis à jour :\n");
    print_clusters(clusters_input, k);

    // Affichage des centroides finaux
    printf("\nCentroides finaux :\n");
    for (int i = 0; i < k; ++i) {
        printf("Centroide %d: ", i);
        print_centroid(clusters_input[i]->centroide);
    }
    printf("\n");

    // Libération de la mémoire allouée pour les clusters et les centroides
    for (int i = 0; i < k; ++i) {
        free(clusters_input[i]->centroide.coords);
