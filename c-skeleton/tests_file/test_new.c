...
void test_update_centroids() {
    ...
    for (uint32_t k = 0; k < 3; k++) {
        clusters[k].size = 3; // Nombre de points dans chaque cluster
    ...
    // Appel de la fonction à tester
    cluster_t centroids = update_centroids(clusters, 3);
    
    // Vérification des résultats
    CU_ASSERT_EQUAL(centroids.size, 3); // Vérifie si le nombre de centroids est correct
